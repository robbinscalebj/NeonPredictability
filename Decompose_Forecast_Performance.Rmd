...
df_gam_oxygen_nested <- df_gam_oxygen|>
  filter(site_id %in% c("BLUE", "ARIK"), forecast_week < 3, forecast_week >0)|>
  mutate(forecast_week = as_factor(forecast_week))|>
  nest_by(forecast_week, site_id)|>
  ungroup()|>
  mutate(batch_name = str_c(site_id, "_",forecast_week),
         batch_number = row_number())

batch_names <- df_gam_oxygen_nested|>pull(batch_name)
#set up options and parallelization
cl <- makePSOCKcluster(5) 
registerDoSNOW(cl)
pb <- txtProgressBar(max = nrow(df_gam_oxygen_nested), style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress = progress)

# Loop
oxy_gams <- foreach(i = 1:nrow(df_gam_oxygen_nested), .combine = 'c', .options.snow = opts, 
        .packages = c("tidyverse", "mgcv"), .inorder = TRUE, .final = function(x) setNames(x, batch_names)) %dopar% {
          
          gam_data <- df_gam_oxygen_nested|>filter(batch_number == i)|>unnest(data)
          
         gam_obj <- gam(data = gam_data, 
    NSE_normed ~ site_id+forecast_week+
                      s(horizon, forecast_week, bs = "sz", by = site_id),
    family = betar)
