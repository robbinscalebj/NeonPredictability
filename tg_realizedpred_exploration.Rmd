...
blah <- download_target("phenology")
...
ggplot(data = tar_hist|>filter(variable == "oxygen"), aes(x = datetime, y = observation))+
  geom_point()+
  facet_grid(.~variable)
# no post 2022 beetle data, only two observations for Amblyomma
...
ggplot(df2|>filter(variable == "nee", is.numeric(R2), R2 >=0), aes(x = horizon, y = R2, color = forecast_month))+
         geom_point()+
  facet_grid(.~forecast_month)

test <- df2|>filter(variable == "abundance")|>filter(!is.na(R2))
test2<-forecasts2|>filter(variable == "abundance", !is.na(observation))

df_test <- df2|>filter(variable == "gcc_90", is.numeric(R2), R2 >=0)|>
  group_by(forecast_month, horizon)|>
  summarize(mean = mean(R2), 
            sd = sd(R2))

ggplot(df_test)+
  geom_point(aes(x = horizon, y = mean))+
