---
title: "Untitled"
output: html_document
date: "2024-01-22"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(tidyverse)
library(here)
library(arrow)
#library(neon4cast)
library(yardstick)
library(marginaleffects)
library(mgcv)
#library(tidymv)
library(tictoc)
library(doParallel)
library(doSNOW)
source(here("download_target.R"))

site_data <- readr::read_csv("https://raw.githubusercontent.com/eco4cast/neon4cast-targets/main/NEON_Field_Site_Metadata_20220412.csv")
```

# Read in forecast metrics
```{r}
crps_scores_df <- read_parquet(here("Data/summarized_crps.parquet"))

nnse_scores_df <- read_parquet(here("Data/summarized_weekly_normedNSE.parquet"))

```

# Data setup

```{r}
nnse_df<- nnse_scores_df|>
  mutate(site_id = as_factor(site_id),
         forecast_week_fct = as_factor(forecast_week))|>
  filter(N_non_na_obs > 4)

rmse_df <- nnse_df|>
  group_by(variable, horizon, site_id, forecast_week)|>
  slice_head(n = 1)
  

crps_df <- crps_scores_df|>#filter(variable == "oxygen")|>
  group_by(variable)|>
  mutate(forecast_month = month(reference_datetime),
         forecast_week = week(reference_datetime))|>
  group_by(variable, horizon, site_id, forecast_week)|>
  summarize(mean_crps_skill = mean(crps_skill, na.rm = TRUE))|>
  mutate(site_id = as_factor(site_id),
         forecast_week_fct = as_factor(forecast_week)) 

ggplot(data = crps_scores_df,aes(crps_skill))+geom_histogram()+scale_x_continuous(limits = c(-2.5,2.5))+facet_wrap(.~variable, scales = "free_y")
```

# CRPS analysis

```{r crps models}
cl <- makeCluster(6)
tic()
crps.temp <- bam(data = crps_df|>
                     filter(variable == "temperature")|>
                     filter(horizon <=30)|>
                     filter(site_id != "BIGC"),#For testing |>filter(forecast_week == 35),
                    mean_crps_skill ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = gaussian(), cluster = cl)
stopCluster(cl)
toc()

cl <- makeCluster(6)
tic()
crps.oxy <- bam(data = crps_df|>
                     filter(variable == "oxygen")|>
                     filter(horizon <=30)|>
                     filter(site_id != "BIGC"),#For testing |>filter(forecast_week == 35),
                    mean_crps_skill ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = gaussian(), cluster = cl)
stopCluster(cl)
toc()

cl <- makeCluster(6)
tic()
crps.chla <- bam(data = crps_df|>
                     filter(variable == "chla")|>
                     filter(horizon <=30)|>
                     filter(site_id != "BIGC"),#For testing |>filter(forecast_week == 35),
                    mean_crps_skill ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = gaussian(), cluster = cl)
stopCluster(cl)
toc()

cl <- makeCluster(6)
tic()
crps.le<- bam(data = crps_df|>
                     filter(variable == "le")|>
                     filter(horizon <=30)|>
                     filter(site_id != "BIGC"),#For testing |>filter(forecast_week == 35),
                    mean_crps_skill ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = gaussian(), cluster = cl)
stopCluster(cl)
toc()

cl <- makeCluster(6)
tic()
crps.nee <- bam(data = crps_df|>
                     filter(variable == "nee")|>
                     filter(horizon <=30)|>
                     filter(site_id != "BIGC"),#For testing |>filter(forecast_week == 35),
                    mean_crps_skill ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = gaussian(), cluster = cl)
stopCluster(cl)
toc()

cl <- makeCluster(6)
tic()
crps.gcc_90 <- bam(data = crps_df|>
                     filter(variable == "gcc_90")|>
                     filter(horizon <=30)|>
                     filter(site_id != "BIGC"),#For testing |>filter(forecast_week == 35),
                    mean_crps_skill ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = gaussian(), cluster = cl)
stopCluster(cl)
toc()

cl <- makeCluster(6)
tic()
crps.rcc_90 <- bam(data = crps_df|>
                     filter(variable == "rcc_90")|>
                     filter(horizon <=30)|>
                     filter(site_id != "BIGC"),#For testing |>filter(forecast_week == 35),
                    mean_crps_skill ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = gaussian(), cluster = cl)
stopCluster(cl)
toc()

```


```{r model checks}
appraise(crps.oxy)
appraise(crps.temp)
appraise(crps.chla)
appraise(crps.nee)
appraise(crps.le)
appraise(crps.gcc_90)
appraise(crps.rcc_90) #weird autocorrelation or something in here

```


```{r plot predictions site by horizon}
horsite_theme<-  theme_classic()+
  theme(legend.position = "none")+
  xlab("Forecast Horizon (days into future)")+
  ylab("Dissolved Oxygen Forecast Skill (CRPS)")

oxy_horizonXsite.pa <- plot_predictions(crps.oxy, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
temp_horizonXsite.pa <- plot_predictions(crps.temp, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
chla_horizonXsite.pa <- plot_predictions(crps.chla, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
nee_horizonXsite.pa <- plot_predictions(crps.nee, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
le_horizonXsite.pa <- plot_predictions(crps.le, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
rcc90_horizonXsite.pa <- plot_predictions(crps.rcc_90, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
gcc90_horizonXsite.pa <- plot_predictions(crps.gcc_90, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme

#plotly::ggplotly(oxy_horizonXsite.pa)

oxy_horizonXsite.pa
temp_horizonXsite.pa
chla_horizonXsite.pa
nee_horizonXsite.pa
le_horizonXsite.pa
rcc90_horizonXsite.pa
gcc90_horizonXsite.pa

```

```{r}
# Coefficient of variation in slope
# Coefficient of variation in skill at different horizons
newdata_horizons <- c(1,5,10,15,20,25,30)
sites_oxy <- crps_df|>ungroup()|>filter(variable == "oxygen")|>
                     filter(site_id != "BIGC")|>distinct(site_id, forecast_week)
sites_temp <- crps_df|>ungroup()|>filter(variable == "temperature")|>
                     filter(site_id != "BIGC")|>distinct(site_id, forecast_week)
sites_chla <- crps_df|>ungroup()|>filter(variable == "chla")|>
                     filter(site_id != "BIGC")|>distinct(site_id, forecast_week)
sites_nee <- crps_df|>ungroup()|>filter(variable == "nee")|>
                     filter(site_id != "BIGC")|>distinct(site_id, forecast_week)
sites_le <- crps_df|>ungroup()|>filter(variable == "le")|>
                     filter(site_id != "BIGC")|>distinct(site_id, forecast_week)
sites_rcc_90 <- crps_df|>ungroup()|>filter(variable == "rcc_90")|>
                     filter(site_id != "BIGC")|>distinct(site_id, forecast_week)
sites_gcc_90 <- crps_df|>ungroup()|>filter(variable == "gcc_90")|>
                     filter(site_id != "BIGC")|>distinct(site_id, forecast_week)



oxy_horizonXsite.preds <- predictions(crps.oxy, newdata = expand_grid(horizon = newdata_horizons, sites_oxy))
temp_horizonXsite.preds <- predictions(crps.temp, newdata = expand_grid(horizon = newdata_horizons, sites_temp))
chla_horizonXsite.preds <- predictions(crps.chla, newdata = expand_grid(horizon = newdata_horizons, sites_chla))
nee_horizonXsite.preds <- predictions(crps.nee, newdata = expand_grid(horizon = newdata_horizons, sites_nee))
le_horizonXsite.preds <- predictions(crps.le, newdata = expand_grid(horizon = newdata_horizons, sites_le))
rcc90_horizonXsite.preds <- predictions(crps.rcc_90,  newdata = expand_grid(horizon = newdata_horizons, sites_rcc_90))
gcc90_horizonXsite.preds <- predictions(crps.gcc_90, newdata = expand_grid(horizon = newdata_horizons,sites_gcc_90))

comb_preds <- bind_rows(oxy_horizonXsite.preds|>mutate(variable = "oxygen"),
                        temp_horizonXsite.preds|>mutate(variable = "temperature"),
                        chla_horizonXsite.preds|>mutate(variable = "chla"),
                        nee_horizonXsite.preds|>mutate(variable = "nee"),
                        le_horizonXsite.preds|>mutate(variable = "le"),
                        gcc90_horizonXsite.preds|>mutate(variable = "gcc_90"),
                        rcc90_horizonXsite.preds|>mutate(variable = "rcc_90"))

comb_preds_sh <- comb_preds|>
  group_by(site_id, horizon, variable)|>
  summarize(estimate = mean(estimate))

comb_preds_sh|>write_csv(here("GAM_crps_skill_summarized.csv"))

# just visualize this worked
ggplot(data = comb_preds, aes(x = horizon, y = estimate, color = site_id))+
  geom_line()+
  facet_wrap(.~variable, scales = "free_y")




```

```{r predict forecast week}
week_errors <- comb_preds|>
  filter(horizon == 5)|>
  group_by(site_id, forecast_week, variable)|>
  summarize(mean_est = median(estimate, na.rm = TRUE), sd_est = sd(estimate, na.rm = TRUE), cv = sd_est/abs(mean_est))|>
  ungroup()|>
  mutate(variable = fct_recode(variable, Temperature = "temperature", Oxygen = "oxygen", RCC90 = "rcc_90", GCC90 = "gcc_90", LE = "le", NEE = "nee", `Chlorophyll a` = "chla"))

var_errors <- comb_preds|>
  filter(horizon == 5)|>
  group_by(site_id, variable)|>
  summarize(mean_est = median(estimate, na.rm = TRUE))|>
  pivot_wider(names_from = "variable", values_from = "mean_est")|>ungroup()|>select(-site_id)
  

GGally::ggpairs(var_errors)


ggplot(week_errors, aes(x = forecast_week, y = mean_est))+
  geom_line(aes(group = site_id))+
  geom_smooth()+
  facet_wrap(.~variable, scales = "free_y")+
  theme_classic()

ggplot(week_errors|>filter(variable == "Oxygen"), aes(x = forecast_week, y = mean_est))+
  geom_line(aes(group = site_id))+
  geom_smooth()+
  facet_wrap(.~variable, scales = "free_y")+
  theme_classic()+
  scale_y_continuous(limits = c(-2,2))

```

Is the skill (crps - climatology) just really dependent on the particular year? 
```{r}
 blah <- comb_preds_sh|>
  group_by(horizon, variable)|>
  summarize(mean_est = median(estimate, na.rm = TRUE), sd_est = sd(estimate, na.rm = TRUE), cv = sd_est/abs(mean_est))|>
  ungroup()|>
  mutate(variable = fct_recode(variable, Temperature = "temperature", Oxygen = "oxygen", RCC90 = "rcc_90", GCC90 = "gcc_90", LE = "le", NEE = "nee", `Chlorophyll a` = "chla"))
levels(blah$variable)

#across-site variation in CRPS-based skill
ggplot(blah|>
  mutate(variable = fct_reorder(variable, desc(cv))), aes(x = variable, y = cv))+geom_point()+
  facet_wrap(.~horizon)+
  ylab("Coefficient of Variation")+
  theme(axis.text.x = element_text(angle = 90))

grey_palette <- c("1" = "grey75", "5" = "grey60", "10" = "grey45", "15" = "grey30", "20" = "grey15", "25" = "grey5", "30" = "black")

#across site means of CRPS skill
ggplot(blah|>
  mutate(variable = fct_reorder(variable, desc(mean_est))))+
  geom_point(aes(x = variable, y = mean_est))+
  #geom_errorbar(aes(x = variable, ymin = mean_est - sd_est, ymax = mean_est + sd_est))+
  facet_wrap(.~horizon)+
  ylab("Mean CRPS")+
  theme(axis.text.x = element_text(angle = 90))


ggplot(blah|>mutate(variable = fct_reorder(variable, desc(mean_est))),
       aes(x = variable, y = mean_est, group = factor(horizon), color = factor(horizon)))+
  geom_point(position = position_dodge(width = 0.6), size = 3) +
  scale_color_manual(values = grey_palette)+
  guides(color = guide_legend(title = "Horizon (days)"))+
  ylab("CRPS Skill (ML vs Climatological)")+
  theme_bw()+theme(axis.title.x = element_blank(), axis.title.y = element_text(size = 16), axis.text = element_text(size = 12), axis.text.x = element_text(angle = 45, vjust = 0.6))

```


# RMSE analysis

```{r crps models}
cl <- makeCluster(6)
tic()
rmse.temp <- bam(data = rmse_df|>
                     filter(variable == "temperature")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc()# 9.2 hours

#saveRDS(rmse.temp, here("rmse_temp_gam.Rds"))

cl <- makeCluster(6)
tic()
rmse.oxy <- bam(data = rmse_df|>
                     filter(variable == "oxygen")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc() #0.5 h

cl <- makeCluster(6)
tic()
rmse.chla <- bam(data = rmse_df|>
                     filter(variable == "chla")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc() # 1 minute

cl <- makeCluster(6)
tic()
rmse.le<- bam(data = rmse_df|>
                     filter(variable == "le")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc() # 50 min

cl <- makeCluster(6)
tic()
rmse.le2<- bam(data = rmse_df|>
                     filter(variable == "le")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      site_id +   
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc() 

cl <- makeCluster(6)
tic()
rmse.nee <- bam(data = rmse_df|>
                     filter(variable == "nee")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc() #~1.5 h

cl <- makeCluster(6)
tic()
rmse.gcc_90 <- bam(data = rmse_df|>
                     filter(variable == "gcc_90")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc() #~40 min

cl <- makeCluster(6)
tic()
rmse.rcc_90 <- bam(data = rmse_df|>
                     filter(variable == "rcc_90")|>
                     filter(horizon <=30),
                    rmse ~ 
                      s(horizon)+  # central trend
                      s(horizon, site_id, bs = "sz")+ #site-level deviations from central trend
                      s(forecast_week, bs = "cc")+ #trend for forecast week
                      ti(horizon, forecast_week)+ #allow effect of horizon to vary based on week
                      s(forecast_week, site_id, bs = "fs", xt= list(bs = "cc")), #site-level deviation for forecast week effect
                    family = Gamma(link = "log"), cluster = cl)
stopCluster(cl)
toc() # 62 minutes

```

```{r model checks}
appraise(rmse.oxy)
appraise(rmse.temp)
appraise(rmse.chla)
appraise(rmse.nee)
appraise(rmse.le)
appraise(rmse.gcc_90)
appraise(rmse.rcc_90)

```




```{r plot predictions site by horizon}

roxy_horizonXsite.pa <- plot_predictions(rmse.oxy, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
rtemp_horizonXsite.pa <- plot_predictions(rmse.temp, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
rchla_horizonXsite.pa <- plot_predictions(rmse.chla, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
rnee_horizonXsite.pa <- plot_predictions(rmse.nee, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
rle_horizonXsite.pa <- plot_predictions(rmse.le, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
rrcc90_horizonXsite.pa <- plot_predictions(rmse.rcc_90, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme
rgcc90_horizonXsite.pa <- plot_predictions(rmse.gcc_90, condition = c("horizon", "site_id"), type = "response")+
  horsite_theme

#plotly::ggplotly(oxy_horizonXsite.pa)

roxy_horizonXsite.pa
rtemp_horizonXsite.pa
rchla_horizonXsite.pa
rnee_horizonXsite.pa
rle_horizonXsite.pa
rrcc90_horizonXsite.pa
rgcc90_horizonXsite.pa

```

```{r}
# Coefficient of variation in slope
# Coefficient of variation in skill at different horizons

roxy_horizonXsite.preds <- predictions(rmse.oxy, newdata = expand_grid(horizon = newdata_horizons, sites_oxy))
rtemp_horizonXsite.preds <- predictions(rmse.temp, newdata = expand_grid(horizon = newdata_horizons, sites_temp))
rchla_horizonXsite.preds <- predictions(rmse.chla, newdata = expand_grid(horizon = newdata_horizons, sites_chla))
rnee_horizonXsite.preds <- predictions(rmse.nee, newdata = expand_grid(horizon = newdata_horizons, sites_nee))
rle_horizonXsite.preds <- predictions(rmse.le, newdata = expand_grid(horizon = newdata_horizons, sites_le))
rrcc90_horizonXsite.preds <- predictions(rmse.rcc_90,  newdata = expand_grid(horizon = newdata_horizons, sites_rcc_90))
rgcc90_horizonXsite.preds <- predictions(rmse.gcc_90, newdata = expand_grid(horizon = newdata_horizons,sites_gcc_90))

comb_rpreds <- bind_rows(roxy_horizonXsite.preds|>mutate(variable = "oxygen"),
                        rtemp_horizonXsite.preds|>mutate(variable = "temperature"),
                        rchla_horizonXsite.preds|>mutate(variable = "chla"),
                        rnee_horizonXsite.preds|>mutate(variable = "nee"),
                        rle_horizonXsite.preds|>mutate(variable = "le"),
                        rgcc90_horizonXsite.preds|>mutate(variable = "gcc_90"),
                        rrcc90_horizonXsite.preds|>mutate(variable = "rcc_90"))

comb_rpreds_sh <- comb_rpreds|>
  group_by(site_id, horizon, variable)|>
  summarize(estimate = mean(estimate))

comb_preds_sh|>write_csv(here("GAM_rmse_summarized.csv"))

# just visualize this worked
ggplot(data = comb_rpreds, aes(x = horizon, y = estimate, color = site_id))+
  geom_line()+
  facet_wrap(.~variable, scales = "free_y")




```

```{r predict forecast week}
week_rerrors <- comb_rpreds|>
  filter(horizon == 5)|>
  group_by(site_id, forecast_week, variable)|>
  summarize(mean_est = median(estimate, na.rm = TRUE), sd_est = sd(estimate, na.rm = TRUE), cv = sd_est/abs(mean_est))|>
  ungroup()|>
  mutate(variable = fct_recode(variable, Temperature = "temperature", Oxygen = "oxygen", RCC90 = "rcc_90", GCC90 = "gcc_90", LE = "le", NEE = "nee", `Chlorophyll a` = "chla"))

var_rerrors <- comb_rpreds|>
  filter(horizon == 5)|>
  group_by(site_id, variable)|>
  summarize(mean_est = median(estimate, na.rm = TRUE))|>
  pivot_wider(names_from = "variable", values_from = "mean_est")|>ungroup()|>select(-site_id)
  

GGally::ggpairs(var_rerrors)


ggplot(week_rerrors, aes(x = forecast_week, y = mean_est))+
  geom_line(aes(group = site_id))+
  geom_smooth()+
  facet_wrap(.~variable, scales = "free_y")+
  theme_classic()

ggplot(week_rerrors|>filter(variable == "Oxygen"), aes(x = forecast_week, y = mean_est))+
  geom_line(aes(group = site_id))+
  geom_smooth()+
  facet_wrap(.~variable, scales = "free_y")+
  theme_classic()+
  scale_y_continuous(limits = c(-2,2))

```




# Old
```{r loops}
# data setup
df_gam_nested <- crps_df|>
  mutate(variable = case_when(variable == "gcc_90" ~ "gcc90", 
                              variable == "rcc_90" ~ "rcc90",
                              .default = variable))|>
  #filter(site_id %in% c("BLUE", "ARIK"), forecast_week < 42, forecast_week >0)|>
  #filter(site_id %in% c("BLUE", "ARIK"), forecast_week == 42)|>
  filter(!is.na(mean_crps_skill))|>
  select(variable,site_id, horizon,forecast_week,mean_crps_skill)|>
  group_by(variable,site_id, forecast_week)|>
  mutate(weekly_points = n())|>
  filter(weekly_points>10)|> 
  ungroup()|>
  mutate(forecast_week = as_factor(forecast_week))|>
  nest_by(variable,forecast_week, site_id)|>
  ungroup()|>
  mutate(batch_name = str_c(variable, "_",site_id, "_",forecast_week),
         batch_number = row_number())

batch_names <- df_gam_nested|>pull(batch_name)
#set up options and parallelization
cl <- makePSOCKcluster(14) 
registerDoSNOW(cl)
pb <- txtProgressBar(max = nrow(df_gam_nested), style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress = progress)

# Loop
tic()
gam_preds <- foreach(i = 1:nrow(df_gam_nested), .combine = 'c', .options.snow = opts, 
        .packages = c("tidyverse", "mgcv","tidymv"), .inorder = TRUE, .final = function(x) setNames(x, batch_names)) %dopar% {
          
          #subset data
          gam_data <- df_gam_nested|>filter(batch_number == i)|>unnest(data)
          
          #fit gam
         gam_obj <- gam(data = gam_data, 
    mean_crps_skill ~ s(horizon),
    family = gaussian)
         
         #extract predictions
         preds <- predict_gam(gam_obj, length_out = 100, 
                       values = list(horizon = seq(0,33, by = 1)))|>
  mutate(crps_fit = fit)|> 
  mutate(upper.se = fit+1.96*se.fit,
         lower.se = fit-1.96*se.fit)|>
           mutate(crps_diff = crps_fit-lag(crps_fit),
                  crps_diff2 = crps_diff-lag(crps_diff))
    
         #extract edf
         edf <- summary(gam_obj)$edf
         
         #bundle up into list
          list(list(#fitted_model = gam_obj, #so short to run, don't need to be saving the model fits
                    edf = edf, 
                    prediction_df = preds))
        }     
stopCluster(cl)
toc() 

```

```{r}

# Wiggliness (edf)
edfs_df <- gam_preds|>
  map(~pluck(., "edf")|>as_tibble_col(column_name = "edf"))|>
  list_rbind(names_to = "id")

preds_df <- gam_preds|>
  map(~pluck(., "prediction_df"))|>
  list_rbind(names_to = "id")
  
p_df <- preds_df|>
  separate_wider_delim(id, "_", names = c("variable", "site","forecast_week"))|>
  mutate(forecast_week = as.numeric(forecast_week))|>
  filter(site == "HARV", variable == "gcc90", forecast_week == "32")|>
  rename(mean_crps_skill = "fit")

ggplot(data = p_df, aes(x = horizon, y = mean_crps_skill))+
  geom_smooth(se=FALSE)+
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = crps_df|>filter(site_id == "HARV", variable == "gcc_90", forecast_week == "32")|>
                      mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = mean_crps_skill))+
  xlab("Forecast Horizon (days)")+
  ylab("CRPS Skill")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))

```


```{r}

#average slope
avg_slope_df <- preds_df|>
  group_by(id)|>
  summarize(avg_1der = mean(crps_diff, na.rm = TRUE),
          crps_0 = crps_fit[horizon == 0],
          crps_5 = crps_fit[horizon == 5],
          crps_10 = crps_fit[horizon == 10],
          crps_15 = crps_fit[horizon == 15],
          crps_33 = crps_fit[horizon == 33],
          )|>
  mutate(crps_change_33 = crps_33-crps_0,
         crps_change_5 = crps_5-crps_0,
         crps_change_10 = crps_10-crps_0) #dividing this by 33 gives exact avg_1der

fn_characteristics_df<-edfs_df|>left_join(avg_slope_df)|>
  separate_wider_delim(id, "_", names = c("variable", "site","forecast_week"))|>
  mutate(forecast_week = as.numeric(forecast_week))


ggplot(fn_characteristics_df, aes(x = edf))+
  geom_density()+
  facet_wrap(.~variable)

ggplot(fn_characteristics_df, aes(x = crps_0))+
  geom_density()+
  facet_wrap(.~variable)

ggplot(fn_characteristics_df, aes(x = crps_change_10))+
  geom_density()+
  facet_wrap(.~variable)

ggplot(fn_characteristics_df, aes(x = avg_1der))+
  geom_density()+
  facet_wrap(.~variable, scales = "free")

```

#Deprecated 

# GAM setup

```{r}
df_gam <- df2|>
  ungroup()|>
  filter(variable %in% c("temperature", "gcc_90"), site_id %in% c("ARIK", "BARR"))|>
  mutate(variable = as_factor(variable),
         site_id = as_factor(site_id))

df_gam_full <- df2|>
  ungroup()|>
  mutate(variable = as_factor(variable),
         site_id = as_factor(site_id))|>
  mutate(variable = case_when(variable == "gcc_90" ~ "gcc90",
                              variable == "rcc_90" ~ "rcc90",
                              .default = variable))
  
  
  

df_test <- df_gam_full|>
  group_split(variable) %>%
   setNames(c(paste0("df_gam_",unique(df_gam_full$variable))))

list2env(df_test, envir = .GlobalEnv)

```

```{r}
cl <- makeCluster(6)
tic()
m3.beta_oxy3 <- bam(data = df_gam_oxygen|>filter(site_id %in% c("BLUE", "ARIK"), forecast_week < 14, forecast_week >0)|>mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ 
                      site_id+
                      s(horizon)+
                      #s(horizon, by = site_id)+
                      s(horizon, forecast_week, bs = "sz", by = site_id),
                    
                      #s(horizon, forecast_week, bs = "sz"),
                      #s(forecast_week, bs= "cc", m = 2)+
                      #s(forecast_week, site_id, bs = "fs", xt = list(bs = "tp"))+
                      #ti(horizon, forecast_week, by = site_id),
                    family = betar,
                    cluster = cl)
toc() # 4 minutes

```
# Oxygen tests
```{r}
cl <- makeCluster(6)
tic()
m3.beta_oxy2 <- bam(data = df_gam_oxygen|>filter(site_id %in% c("BLUE", "ARIK"), forecast_week < 14, forecast_week >0)|>mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ 
                      site_id+forecast_week+
                      s(horizon)+
                      #s(horizon, by = site_id)+
                      s(horizon, forecast_week, bs = "sz", by = site_id),
                    
                      #s(horizon, forecast_week, bs = "sz"),
                      #s(forecast_week, bs= "cc", m = 2)+
                      #s(forecast_week, site_id, bs = "fs", xt = list(bs = "tp"))+
                      #ti(horizon, forecast_week, by = site_id),
                    family = betar,
                    cluster = cl)
toc() # 4 minutes

tic()
m3.beta_oxy3 <- bam(data = df_gam_oxygen|>filter(site_id %in% c("BLUE", "ARIK"), forecast_week < 14, forecast_week >0)|>mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ 
                      site_id+forecast_week+
                      #s(horizon)+ #average forecast across sites
                      #s(horizon, by = forecast_week)+ #average forecast across sites for each week
                      #s(horizon, by = site_id)+ #average forecast by site
                      s(horizon, forecast_week, bs = "sz", by = site_id), #
                    
                      #s(horizon, forecast_week, bs = "sz"),
                      #s(forecast_week, bs= "cc", m = 2)+
                      #s(forecast_week, site_id, bs = "fs", xt = list(bs = "tp"))+
                      #ti(horizon, forecast_week, by = site_id),
                    family = betar,
                    cluster = cl)
toc() #  minutes

m3.beta_oxy4 <- gam(data = df_gam_oxygen|>filter(site_id %in% c("BLUE"), forecast_week < 14, forecast_week >0)|>mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ 
                      forecast_week+
                      s(horizon, by = forecast_week, k = 30), 
                    family = betar)

oxy_pred <- predict_gam(m3.beta_oxy4, length_out = 100, 
                       values = list(horizon = seq(0,30, by = 1), 
                                     forecast_week = seq(1,13, by = 1)))|>
  mutate(NSE_normed = exp(fit)/(1+exp(fit)))|> #inverse logit, which is link function
  mutate(upper.se = exp(fit+1.96*se.fit)/(1+exp(fit+1.96*se.fit)),
         lower.se = exp(fit-1.96*se.fit)/(1+exp(fit-1.96*se.fit)))


ggplot(data = oxy_pred|>
         filter(forecast_week >0, forecast_week <11), aes(x = horizon, y = NSE_normed))+
  #geom_point(color = "red")+
  geom_smooth(se=FALSE)+
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_oxygen|>filter(site_id %in% c("BLUE"))|>filter(forecast_week >0, forecast_week <11)|>mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
   #geom_smooth(data = df_gam_oxygen|>filter(site_id %in% c("BLUE"))|>filter(forecast_week >0, forecast_week <11)|>mutate(forecast_week = as_factor(forecast_week)), 
    #         aes(x = horizon, y = NSE_normed))+
  facet_wrap(.~forecast_week)+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))
```

# Looped independent GAMs
```{r check single site and week GAM}

#Temperature 
m3.beta_temp1 <- gam(data = df_gam_full|>filter(site_id == "TOMB", forecast_week == 1, variable == "chla")|>
                      mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ s(horizon), 
                    family = betar)


temp_pred <- predict_gam(m3.beta_temp1, length_out = 100, 
                       values = list(horizon = seq(0,33, by = 1)))|>
  mutate(NSE_normed = exp(fit)/(1+exp(fit)))|> #inverse logit, which is link function
  mutate(upper.se = exp(fit+1.96*se.fit)/(1+exp(fit+1.96*se.fit)),
         lower.se = exp(fit-1.96*se.fit)/(1+exp(fit-1.96*se.fit)))

ggplot(data = temp_pred, aes(x = horizon, y = NSE_normed))+
  geom_smooth(se=FALSE)+
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_full|>filter(site_id == "TOMB", forecast_week == 1,variable == "chla")|>
                      mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))

#Oxygen

m3.beta_oxy1 <- gam(data = df_gam_full|>filter(site_id == "ARIK", forecast_week == 1, variable == "temperature")|>
                      mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ s(horizon, k = 5), 
                    family = betar)

blah <- list(m3.beta_oxy1)


oxy_pred <- predict_gam(m3.beta_oxy1, length_out = 100, 
                       values = list(horizon = seq(0,33, by = 1)))|>
  mutate(NSE_normed = exp(fit)/(1+exp(fit)))|> #inverse logit, which is link function
  mutate(upper.se = exp(fit+1.96*se.fit)/(1+exp(fit+1.96*se.fit)),
         lower.se = exp(fit-1.96*se.fit)/(1+exp(fit-1.96*se.fit)))

ggplot(data = oxy_pred, aes(x = horizon, y = NSE_normed))+
  geom_smooth(se=FALSE)+
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_full|>filter(site_id == "ARIK", forecast_week == 1, variable == "temperature")|>
                      mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))



```

```{r loops}
# data setup
df_gam_nested <- df_gam_full|>
  #filter(site_id %in% c("BLUE", "ARIK"), forecast_week < 42, forecast_week >0)|>
  #filter(site_id %in% c("BLUE", "ARIK"), forecast_week == 42)|>
  select(variable,site_id, horizon,forecast_week,NSE_normed, N_non_na_obs)|>
  filter(N_non_na_obs>2)|>
  group_by(variable,site_id, forecast_week)|>
  mutate(weekly_points = n())|>
  filter(weekly_points>20)|> #reduced dataset by ~15 %
  ungroup()|>
  mutate(forecast_week = as_factor(forecast_week))|>
  nest_by(variable,forecast_week, site_id)|>
  ungroup()|>
  mutate(batch_name = str_c(variable, "_",site_id, "_",forecast_week),
         batch_number = row_number())

batch_names <- df_gam_nested|>pull(batch_name)
#set up options and parallelization
cl <- makePSOCKcluster(14) 
registerDoSNOW(cl)
pb <- txtProgressBar(max = nrow(df_gam_nested), style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress = progress)

# Loop
tic()
gam_preds <- foreach(i = 1:nrow(df_gam_nested), .combine = 'c', .options.snow = opts, 
        .packages = c("tidyverse", "mgcv","tidymv"), .inorder = TRUE, .final = function(x) setNames(x, batch_names)) %dopar% {
          
          #subset data
          gam_data <- df_gam_nested|>filter(batch_number == i)|>unnest(data)
          
          #fit gam
         gam_obj <- gam(data = gam_data, 
    NSE_normed ~ s(horizon),
    family = betar)
         
         #extract predictions
         preds <- predict_gam(gam_obj, length_out = 100, 
                       values = list(horizon = seq(0,33, by = 1)))|>
  mutate(NSE_normed_fit = exp(fit)/(1+exp(fit)))|> #inverse logit, which is link function
  mutate(upper.se = exp(fit+1.96*se.fit)/(1+exp(fit+1.96*se.fit)),
         lower.se = exp(fit-1.96*se.fit)/(1+exp(fit-1.96*se.fit)))|>
           mutate(NSE_diff = NSE_normed_fit-lag(NSE_normed_fit),
                  NSE_diff2 = NSE_diff-lag(NSE_diff))
    
         #extract edf
         edf <- summary(gam_obj)$edf
         
         #bundle up into list
          list(list(#fitted_model = gam_obj, #so short to run, don't need to be saving the model fits
                    edf = edf, 
                    prediction_df = preds))
        }     
stopCluster(cl)
toc() # 1.5 minute to run all on 4 cores - 40s on 14 cores


# Loop but gam has ar1 strcutrue on resids
tic()
gam_preds <- foreach(i = 1:nrow(df_gam_nested), .combine = 'c', .options.snow = opts, 
        .packages = c("tidyverse", "mgcv","tidymv"), .inorder = TRUE, .final = function(x) setNames(x, batch_names)) %dopar% {
          
          #subset data
          gam_data <- df_gam_nested|>filter(batch_number == i)|>unnest(data)
          
          #fit gam
         gam_obj <- gamm(data = gam_data, 
    NSE_normed ~ s(horizon),
    correlation = corAR1(form = ~horizon),
    family = betar)
         list(gam_obj)
        }
         #extract predictions
         preds <- predict_gam(gam_obj$gam, length_out = 100, 
                       values = list(horizon = seq(0,33, by = 1)))|>
  mutate(NSE_normed_fit = exp(fit)/(1+exp(fit)))|> #inverse logit, which is link function
  mutate(upper.se = exp(fit+1.96*se.fit)/(1+exp(fit+1.96*se.fit)),
         lower.se = exp(fit-1.96*se.fit)/(1+exp(fit-1.96*se.fit)))|>
           mutate(NSE_diff = NSE_normed_fit-lag(NSE_normed_fit),
                  NSE_diff2 = NSE_diff-lag(NSE_diff))
    
         #extract edf
         edf <- summary(gam_obj)$edf
         
         #bundle up into list
          list(list(#fitted_model = gam_obj, #so short to run, don't need to be saving the model fits
                    edf = edf, 
                    prediction_df = preds))
        }     
stopCluster(cl)
toc()
```

```{r extract info from GAM list and calculate functional characteristics}


# Wiggliness (edf)
edfs_df <- gam_preds|>
  map(~pluck(., "edf")|>as_tibble_col(column_name = "edf"))|>
  list_rbind(names_to = "id")

preds_df <- gam_preds|>
  map(~pluck(., "prediction_df"))|>
  list_rbind(names_to = "id")

#average slope
avg_slope_df <- preds_df|>#slice_head(n = 102)|>
  group_by(id)|>
  summarize(avg_1der = mean(NSE_diff, na.rm = TRUE),
          nse_0 = NSE_normed_fit[horizon == 0],
          nse_5 = NSE_normed_fit[horizon == 5],
          nse_10 = NSE_normed_fit[horizon == 10],
          nse_15 = NSE_normed_fit[horizon == 15],
          nse_33 = NSE_normed_fit[horizon == 33],
          )|>
  mutate(nse_change_33 = nse_33-nse_0,
         nse_change_5 = nse_5-nse_0,
         nse_change_10 = nse_10-nse_0) #dividing this by 33 gives exact avg_1der

fn_characteristics_df<-edfs_df|>left_join(avg_slope_df)|>
  separate_wider_delim(id, "_", names = c("variable", "site","forecast_week"))|>
  mutate(forecast_week = as.numeric(forecast_week))


ggplot(fn_characteristics_df, aes(x = edf))+
  geom_density()+
  facet_wrap(.~variable)

ggplot(fn_characteristics_df, aes(x = nse_0))+
  geom_density()+
  facet_wrap(.~variable)

ggplot(fn_characteristics_df, aes(x = nse_change_10))+
  geom_density()+
  facet_wrap(.~variable)

ggplot(fn_characteristics_df, aes(x = avg_1der))+
  geom_density()+
  facet_wrap(.~variable)
```

```{r summaries}


fn_characteristics_sums<-fn_characteristics_df|>
  group_by(variable)|>
  summarize(across(-c(site,forecast_week), list(mean = mean, sd = sd)), 
            above_0_5 = sum(nse_change_5 >=0)/n(),
            above_0_10 = sum(nse_change_10 >=0)/n(),
            above_0_33 = sum(nse_change_33 >=0)/n())

ggplot(fn_characteristics_sums)+
  geom_point(aes(x = variable, y = nse_change_mean))+
  geom_errorbar(aes(x = variable, ymin = nse_change_mean-nse_change_sd, ymax = nse_change_mean+nse_change_sd))

ggplot(fn_characteristics_sums)+
  geom_point(aes(x = variable, y = nse_0_mean))+
  geom_errorbar(aes(x = variable, ymin = nse_0_mean-nse_0_sd, ymax = nse_0_mean+nse_0_sd))

ggplot(fn_characteristics_sums)+
  geom_point(aes(x = variable, y = nse_10_mean))+
  geom_errorbar(aes(x = variable, ymin = nse_10_mean-nse_10_sd, ymax = nse_10_mean+nse_10_sd))

ggplot(fn_characteristics_sums)+
  geom_point(aes(x = variable, y = nse_33_mean))+
  geom_errorbar(aes(x = variable, ymin = nse_33_mean-nse_33_sd, ymax = nse_33_mean+nse_33_sd))
```

```{r decompose variation across spatial/temporal dimensions}

#for ease, start with NSE at horizon0 - what's more important, variation in space or time?
sums_across_temporal <- fn_characteristics_df|>
  group_by(site,variable)|>
  summarise(mean_nse_0 = mean(nse_0), sd_nse_0 = sd(nse_0))

sums_across_space <- fn_characteristics_df|>
  group_by(forecast_week,variable)|>
  summarise(mean_nse_0 = mean(nse_0), sd_nse_0 = sd(nse_0))

range_across_space <- sums_across_space|>
  group_by(variable)|>
  summarize(mean = mean(mean_nse_0),sd = sd(mean_nse_0))

range_across_time <- sums_across_temporal|>
  group_by(variable)|>
  summarize(mean = mean(mean_nse_0),sd = sd(mean_nse_0))

balh.lm <- lm(data = fn_characteristics_df|>filter(variable == "nee"), nse_0 ~ site+as_factor(forecast_week))

anova(balh.lm)

```
```{r plot example fits}

p_df <- preds_df|>
  separate_wider_delim(id, "_", names = c("variable", "site","forecast_week"))|>
  mutate(forecast_week = as.numeric(forecast_week))|>
  filter(site == "HARV", variable == "gcc90", forecast_week == "32")|>
  rename(NSE_normed = "NSE_normed_fit")

ggplot(data = p_df, aes(x = horizon, y = NSE_normed))+
  geom_smooth(se=FALSE)+
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_full|>filter(site_id == "HARV", variable == "gcc90", forecast_week == "32")|>
                      mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))

```
######## Old #########

```{r}
oxy_pred <- predict_gam(m3.beta_oxy3, length_out = 100, 
                       values = list(#site_id = unique(df_gam_oxygen$site_id),
                         site_id = c("BLUE", "ARIK"),
                                     horizon = seq(0,30, by = 1), 
                                     forecast_week = seq(1,max(df_gam_oxygen$forecast_week)), by = 1))|>
  #left_join(df_gam|>group_by(variable, site_id, horizon, forecast_week)|>summarize(mean_nse = mean(NSE_normed,na.rm = TRUE)))
  #mutate(variable = fct_recode(variable,`Chlorophyll-a`= "chla", Greenness = "gcc_90", Redness = "rcc_90", Evapotranspiration = "le", `Net Ecosystem Exchange` = "nee", `Dissolved O2` = "oxygen",  `Water Temp` = "temperature"))|>
  mutate(NSE_normed = exp(fit)/(1+exp(fit)))|> #inverse logit, which is link function
  #mutate(forecast_week = as_factor(forecast_week))|>
  mutate(upper.se = exp(fit+1.96*se.fit)/(1+exp(fit+1.96*se.fit)),
         lower.se = exp(fit-1.96*se.fit)/(1+exp(fit-1.96*se.fit)))


ggplot(data = oxy_pred|>filter(site_id %in% c("BLUE"))|>
         filter(forecast_week >0, forecast_week <11), aes(x = horizon, y = NSE_normed))+
  #geom_point(color = "red")+
  geom_smooth(se=FALSE)+
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_oxygen|>filter(site_id %in% c("BLUE"))|>filter(forecast_week >0, forecast_week <11)|>mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
   #geom_smooth(data = df_gam_oxygen|>filter(site_id %in% c("BLUE"))|>filter(forecast_week >0, forecast_week <11)|>mutate(forecast_week = as_factor(forecast_week)), 
    #         aes(x = horizon, y = NSE_normed))+
  facet_wrap(.~forecast_week)+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))

ggplot(data = oxy_pred|>filter(site_id %in% c("ARIK"))|>
         filter(forecast_week >0, forecast_week <11), aes(x = horizon, y = NSE_normed))+
  #geom_point(color = "red")+
  geom_smooth(se=FALSE)+
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_oxygen|>filter(site_id %in% c("ARIK"))|>filter(forecast_week >0, forecast_week <11, horizon <31)|>mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
   #geom_smooth(data = df_gam_oxygen|>filter(site_id %in% c("BLUE"))|>filter(forecast_week >0, forecast_week <11)|>mutate(forecast_week = as_factor(forecast_week)), 
    #         aes(x = horizon, y = NSE_normed))+
  facet_wrap(.~forecast_week)+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))

```

#Calculate functional shape characteristics
## Estimate first derivative

```{r}
temp_derivs<-temp_pred|>
  group_by(site_id, forecast_week)|>
  mutate(NSE_diff = NSE_normed-lag(NSE_normed),
         NSE_diff2 = NSE_diff-lag(NSE_diff))

ggplot(data = temp_derivs|>filter(site_id %in% c("ARIK"))|>
         filter(forecast_week >0, forecast_week <11), aes(x = horizon, y = NSE_diff))+
  geom_smooth(se=FALSE)+  facet_wrap(.~forecast_week)+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))


ggplot(data = temp_derivs|>filter(site_id %in% c("BLUE"))|>
         filter(forecast_week >0, forecast_week <11), aes(x = horizon, y = NSE_diff))+
  geom_smooth(se=FALSE)+  facet_wrap(.~forecast_week)+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
  theme_bw()+
  theme(legend.position  = c(0.75,0.15), legend.title = element_blank(),
        axis.title.y = element_text(size = 18),axis.title.x = element_text(size = 18, face = "bold"),
        axis.text.x = element_text(face = "bold"), axis.text.y = element_text(face = "bold"),
        strip.text = element_text(face = "bold", size = 12), legend.text = element_text(size = 10, face = "bold"))+
  guides(color=guide_legend(nrow=4))


deriv_summaries <- temp_derivs|>
  group_by(site_id, forecast_week)|>
  summarize(avg_1st_deriv = mean(NSE_diff, na.rm = TRUE),
            avg_2nd_deriv = mean(NSE_diff2, na.rm = TRUE),
            sum_2nd_deriv = sum(NSE_diff2, na.rm = TRUE))|>
  filter(!is.na(avg_1st_deriv))

deriv_summaries2<-deriv_summaries|>
  group_by(site_id)
```



# 