...
aq_scores_test <- aq_scores|>filter(horizon >= 0, horizon<35)|>filter(site_id == "BARC", variable %in% c("temperature", "chla"))|>
  #group_by(model_id)|>count()
  mutate(reference_datetime = as_date(reference_datetime))
 
crps_scores_df <- aq_scores_test|>
...
ggplot(aq_scores_test|>filter(variable == "chla",horizon == 0), aes(x = reference_datetime, y = observation))+geom_point()
                                filter(reference_datetime>=as_datetime("2023-01-18")&reference_datetime<=as_datetime("2023-01-22"))
