...
#Temperature 
m3.beta_temp1 <- gam(data = df_gam_temperature|>filter(site_id == "ARIK", forecast_week == 14)|>
                      mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ horizon + s(horizon, k = 5), 
                    family = betar)
...
ggplot(data = temp_pred, aes(x = horizon, y = NSE_normed))+
  ...
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_temperature|>filter(site_id == "ARIK", forecast_week == 14)|>
                      mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
...
m3.beta_oxy1 <- gam(data = df_gam_oxygen|>filter(site_id == "ARIK", forecast_week == 1)|>
                      mutate(forecast_week = as_factor(forecast_week)),
...
ggplot(data = oxy_pred, aes(x = horizon, y = NSE_normed))+
  ...
  geom_smooth(aes(x = horizon, y = upper.se),se = FALSE, color = "red")+
  geom_smooth(aes(x = horizon, y = lower.se),se = FALSE, color = "red")+
  geom_point(data = df_gam_oxygen|>filter(site_id == "ARIK", forecast_week == 1)|>
                      mutate(forecast_week = as_factor(forecast_week)), 
             aes(x = horizon, y = NSE_normed))+
  xlab("Forecast Horizon (days)")+
  ylab("Normed NSE")+
...
stopCluster(cl)
toc() # 1.5 minute to run all

```

```{r extract info from GAM list}
...
avg_2deriv_df <- preds_df|>
  group_by(id)|>summarize(avg_1der = mean(NSE_diff, na.rm = TRUE),
                          sum_2deriv = sum(abs(NSE_diff2), na.rm = TRUE),
                          sum_1deriv = sum(abs(NSE_diff), na.rm = TRUE))

blah<-edfs_df|>left_join(avg_2deriv_df)|>
  separate_wider_delim(id, "_", names = c("variable", "site","forecast_week"), too_many = "debug")|>
  mutate(forecast_week = as.numeric(forecast_week))

ggplot(blah, aes(x = edf, y = sum_1deriv))+
  geom_point()+
  geom_smooth()

ggplot(blah, aes(x = avg_1der))+
  geom_density()+
  facet_wrap(.~variable)

ggplot(blah|>filter(variable == "temperature")|>filter(site == "ARIK"), 
       aes(x = forecast_week, y = avg_1der))+
  geom_point()
```
...
cl <- makeCluster(4)
tic()
m3.beta_temp <- bam(data = df_gam_temperature|>filter(site_id %in% c("BLUE", "ARIK","KING", "LECO", "WALK", "TOOK"), forecast_week < 14, forecast_week >0)|>mutate(forecast_week = as_factor(forecast_week)),
                    NSE_normed ~ 
                      site_id+
                      s(horizon)+
                      #s(horizon, by = site_id)+
...
```{r try to estimate average forecast per site}
temp_pred2 <- predict_gam(m3.beta_temp, length_out = 100, exclude_terms
                       values = list(#site_id = unique(df_gam_oxygen$site_id),
                         site_id = c("BLUE", "ARIK"),
                                     horizon = seq(0,30, by = 1), 
                                     forecast_week = seq(1,max(df_gam_oxygen$forecast_week)), by = 1))|>
